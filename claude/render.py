#!/usr/bin/env python3
"""Build the Claude Code cheat sheet as paginated HTML, PDF, and preview PDF."""
...
OUTPUT_PREFIX = "Claude-Code-Cheat-Sheet"
...
def parse_source(path: Path) -> tuple[str, str, str]:
    ...
    version_match = re.search(r"^Claude Code Cheat Sheet version\s+(.+?)\s*$", source, re.MULTILINE)
    ...
    content = re.sub(
        r"\n---\s*\n\s*Claude Code Cheat Sheet version[\s\S]*$",
        "",
        content,
    ).strip()
    content = re.sub(
...
<body class="{esc(variant)}">
  <section class="book-page cover-page" data-page-id="cover">
    ...
    <div class="cover-copy">
      ...
      <h1><span>Claude Code</span><br><em>Cheat Sheet</em></h1>
      <p>Commands, context, automation, and workflows for productive agentic coding.</p>
    </div>
  ...
  <script>
    (() => {{
      ...
      function newPage() {{
        ...
        page.innerHTML = `<header class="running-head"><span>CLAUDE CODE</span><b>${{String(pageNumber).padStart(2, '0')}}</b></header><div class="page-body"></div><footer><span>CHEAT SHEET</span><span>ANDREI SMIRNOV</span></footer>`;
